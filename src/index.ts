...
function eventStreamToReadableStream(eventStream: EventStream<ChatStreamingResponseChunkData>): ReadableStream<Uint8Array> {
		...
		if (url.pathname === "/api/chat") {
			...
			if (request.method === "POST") {
				...
				if (result.ok) {
					...
					return new Response(eventStreamToReadableStream(result.value as EventStream<ChatStreamingResponseChunkData>), {
						status: 200,
